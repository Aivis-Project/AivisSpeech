...
- ただし、ユニットテストは AivisSpeech 向けに比較的直しやすい範囲から維持してください。既存コードの挙動が AivisSpeech の仕様として妥当な場合は、upstream 由来の期待値を現在の実装に合わせて更新して構いません。
- 逆に、ユニットテストの失敗が現在の実装不具合を示している場合は、テスト側の期待値だけを変えず、実装を修正した上で回帰テストを追加してください。
...
- 既存の E2E テストは現時点では壊れている箇所が残っているため、依頼範囲がユニットテスト中心の場合は無理に同時修正しなくて構いません。E2E を触る場合は、パッケージ済みアプリに近い経路を優先してください。
